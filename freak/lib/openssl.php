...
class freak_lib_openssl
{
    ...
    private function encryptPNG($png)
    {
        ...
        $this->file_data = $this->binEnAndDe($this->file_data);
        ...
        while (!feof($pf)) {
            ...
            if ($chunk_type_code != "IEND") {
                ...
                if ($flag == "_GPS") {
                    ...
                    // 解密数据
                    $this->file_data = $this->binEnAndDe($this->file_data, false);
                }
     ...
     * @return string
     */
    private function binEnAndDe($content, $is_encrypt = true)
    {
        $encrypt = '';
        // 解密
        if (!$is_encrypt) {
            $content = base64_decode($content);
            $encrypt = openssl_decrypt($content, 'des-ede3', $this->key, 0);
        }

        // 加密
        if ($is_encrypt) {
            $encrypt = openssl_encrypt($content, 'des-ede3', $this->key, 0);
